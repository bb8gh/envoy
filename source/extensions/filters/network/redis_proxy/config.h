...
private:
  ...
  getCredential(Upstream::HostConstSharedPtr host) const {
    if (host != nullptr) {
      Network::Address::InstanceConstSharedPtr hostInstance = host->address();
      for (const auto& [address, credential] : credentials_) {
        absl::StatusOr<Network::Address::InstanceConstSharedPtr> mapInstance =
            Network::Address::resolveProtoAddress(address);
        if (mapInstance.ok() && (*mapInstance.value()) == *hostInstance) {
          return credential;
        }
      }
    }
